...
def process_all_records(records):
    ...
    for record in records:
        ...
        process_record(hopper_bucket, object_key, result_bucket, result_postfix, archive_bucket)

def process_record(hopper_bucket, object_key, result_bucket, result_postfix, archive_bucket):
    local_source_filepath = '/tmp/{}'.format(uuid.uuid4())
    s3_client.download_file(hopper_bucket, object_key, local_filepath)

    local_result_filepath = '/tmp/{}'.format(uuid.uuid4())
    task.do_task(local_source_filepath, local_result_filepath):
